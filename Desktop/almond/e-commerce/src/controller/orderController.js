...
const orderPlace = async function (req, res) {
  ...
  //console.log(userFind.balance);
  if (!userFind)
    return res
      .status(400)
      .send({ status: false, message: "Before placing the order you need to first register" });
  ...
  //console.log(productFind.quentity);
  ...
  // let orderFinded = await Orders.findOne({
  //   where:{
  //     user_Id:userId,
  ...
  //   }
  // })
  // if(!orderFinded) return res.status(400).send({status:false,message:"before placing the order you need to add this prodcut in your cart"})
  ...
  if(!findAmount) return res.status(400).send({status:false,message:"before placing the order you need to add this prodcut in your cart"})
  ...
  //orderFind.users.orderPrice = 'y*z'
  let a = x - y * z;
  //console.log(a)
  if (a < 0) {
  ...
  let updateUserBalance = await Orders.findOne({
    ...
    if(updateUserBalance){
      ...
      return res
        ...
        .send({
          status: false,
          message: "product order successfully and amount is also credit in account and product also update in stock",
        });
    }
    }
    else{
      return res.status(400).send({status:400,message:"user are not able to return this order because you are not a valid user for returning this product"})
    }
};
